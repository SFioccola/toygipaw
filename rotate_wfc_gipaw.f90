...
!
!------------------------------------------------------------------------------------
SUBROUTINE rotate_wfc_gipaw ( npwx, npw, nstart, gstart, nbnd, psi, npol, overlap, evc, e )
  !--------------------------------------------------------------------------------
...
!  print*, 'chiama rotat_wfc_gipaw'
  ...
  IF( use_para_diag ) THEN
     ...
     IF ( gamma_only ) THEN
        ...
        !
        CALL protate_wfc_gamma ( h_psi_gipaw, s_psi, overlap, &
                                 npwx, npw, nstart, nbnd, psi, evc, e )
        !
     ELSE
        !write (*,*) 'inside para k'; FLUSH(6)
        !
        CALL protate_wfc_k( h_psi_gipaw, s_psi, overlap, &
  ...
  ELSE
     ...
     IF ( gamma_only ) THEN
        ...
        !
        CALL rotate_wfc_gamma( h_psi_gipaw, s_psi, overlap, &
                               npwx, npw, nstart, nbnd, psi, evc, e )
        !
     ELSE
  !write (*,*) 'inside serial k'; FLUSH(6)
        !
        CALL rotate_wfc_k_gipaw ( h_psi_gipaw, s_psi, overlap, &
                           npwx, npw, nstart, nbnd, npol, psi, evc, e )
        !
     ENDIF
     !
  ...
  !
END SUBROUTINE rotate_wfc_gipaw
