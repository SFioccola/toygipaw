...
SUBROUTINE c_bands_gipaw ( iter )
  ...
  LOGICAL :: exst
  LOGICAL,EXTERNAL :: rmm_use_davidson_gipaw, rmm_use_paro_gipaw
  !
  INTEGER :: ierr
  !
  !
...
!  print*, 'chiama c_bands_gipaw'
  ...
  ELSEIF ( isolve == 4 ) THEN
     IF (rmm_use_davidson_gipaw(iter)) THEN 
       WRITE( stdout, '(5X,"Davidson diagonalization with overlap")' )
     ELSE IF (rmm_use_paro_gipaw(iter)) THEN 
      WRITE( stdout, '(5X,"ParO style diagonalization")')
     ELSE 
...
!     print*, 'chiama g2_kin'
  ...
  RETURN
  !
END SUBROUTINE c_bands_gipaw
!
!----------------------------------------------------------------------------
SUBROUTINE diag_bands_gipaw( iter, ik, avg_iter )
  !----------------------------------------------------------------------------
 ...
 ! m = nbnd
  ...
  IF ( gamma_only ) THEN
     !
     CALL diag_bands_gamma_gipaw()
     !
  ELSE
     !
     CALL diag_bands_k_gipaw()
     !
  ENDIF
  !
 ...
 CONTAINS
  ...
  !
  !-----------------------------------------------------------------------
  SUBROUTINE diag_bands_gamma_gipaw()
    !-----------------------------------------------------------------------
    !
...
!                        print*,'entra da c_bands1'
          ...
          ELSE IF ( isolve == 2 ) THEN
             IF (.not. use_gpu) THEN
               CALL using_evc(1);  CALL using_et(1); CALL using_h_diag(0) ! precontidtion has intent(in)
               CALL ppcg_gamma( h_psi_gipaw, s_psi, okvan, h_diag, &
                           npwx, npw, nbnd, evc, et(1,ik), btype(1,ik), &
          ...
          ELSE
             ...
             IF (.not. use_gpu ) THEN
               CALL using_evc(1);  CALL using_et(1); CALL using_h_diag(0) ! precontidtion has intent(in)
               CALL paro_gamma_new( h_psi_gipaw, s_psi, hs_psi, g_1psi, okvan, &
          ...
          ! ... exit condition
          !
          IF ( test_exit_cond_gipaw() ) EXIT  CG_loop
          !
       ENDDO CG_loop
       !
    ELSE IF ( isolve == 4 .AND. .NOT. rmm_use_davidson_gipaw(iter)) THEN
       !
...
!              CALL using_h_diag(0)
          ...
          IF (.not. use_gpu) THEN
            CALL using_evc(1);  CALL using_et(1); CALL using_h_diag(0) !precontidtion has intent(in)
            CALL rrmmdiagg( h_psi_gipaw, s_psi, npwx, npw, nbnd, evc, hevc, sevc, &
          ...
          ! ... exit condition
          !
          IF ( test_exit_cond_gipaw() ) EXIT  RMM_loop
          !
       END DO RMM_loop
       !
...
!                ! make sure that all processors have the same wfc
          ...
          ! ... exit condition
          !
          IF ( test_exit_cond_gipaw() ) EXIT  david_loop
          !
       ENDDO david_loop
       !
    ENDIF
    ...
    RETURN
    !
  END SUBROUTINE diag_bands_gamma_gipaw
  !
  !-----------------------------------------------------------------------
  SUBROUTINE diag_bands_k_gipaw()
    !-----------------------------------------------------------------------
       ...
       CG_loop : DO
          ...
          ELSE IF ( isolve == 2) then
             IF ( .not. use_gpu ) THEN
               ...
               CALL ppcg_k( h_psi_gipaw, s_psi, okvan, h_diag, &
                           npwx, npw, nbnd, npol, evc, et(1,ik), btype(1,ik), &
                           0.1d0*ethr, max_ppcg_iter, notconv, ppcg_iter, sbsize , rrstep, iter )
               !
               avg_iter = avg_iter + ppcg_iter
          ...
          ELSE
             ...
             IF ( .not. use_gpu ) THEN
               CALL using_evc(1); CALL using_et(1); CALL using_h_diag(0)
               CALL paro_k_new( h_psi_gipaw, s_psi, hs_psi, g_1psi, okvan, &
...
!          IF ( .NOT. lrot ) THEN
          ...
          IF ( .not. use_gpu ) THEN
             CALL using_evc(1); CALL using_et(1); CALL using_h_diag(0)
             CALL crmmdiagg( h_psi_gipaw, s_psi, npwx, npw, nbnd, npol, evc, hevc, sevc, &
                             et(1,ik), g2kin(1), btype(1,ik), ethr, rmm_ndim, &
                             okvan, lrot, exx_is_active(), notconv, rmm_iter )
          ELSE
...
!          print*, 'npol', npol
       ...
       david_loop: DO
          ...
          IF (.not. use_gpu ) THEN
             ...
             IF ( use_para_diag ) then
                !
                CALL pcegterg( h_psi_gipaw, s_psi, okvan, g_psi, &
...
!                print*, 'si blocca qui'
    ...
    RETURN
    !
  END SUBROUTINE diag_bands_k_gipaw
  !
  !-----------------------------------------------------------------------
  FUNCTION test_exit_cond_gipaw()
    !-----------------------------------------------------------------------
    ...
    IF ( lscf .AND. lgcscf ) THEN
       ...
       !
       test_exit_cond_gipaw = .NOT. ( ( ntry <= 8 ) .AND. ( notconv > 0 ) )
       !
    ELSE
       !
       test_exit_cond_gipaw = .NOT. ( ( ntry <= 5 ) .AND. &
            ( ( .NOT. lscf .AND. ( notconv > 0 ) ) .OR. &
            (       lscf .AND. ( notconv > 5 ) ) ) )
       !
    END IF
    !
  END FUNCTION test_exit_cond_gipaw
  !
END SUBROUTINE diag_bands_gipaw
!
!----------------------------------------------------------------------------
SUBROUTINE c_bands_efield_gipaw( iter )
  !----------------------------------------------------------------------------
  ...
  RETURN
  !
END SUBROUTINE c_bands_efield_gipaw
!
!------------------------------------------------------------------------------
SUBROUTINE c_bands_nscf_gipaw( )
  !----------------------------------------------------------------------------
  ...
  k_loop: DO ik = ik_+1, nks
     ...
     ! ... diagonalization of bands for k-point ik
     !
     CALL diag_bands_gipaw( 1, ik, avg_iter )
     !
...
FUNCTION rmm_use_davidson_gipaw(iter_) RESULT (res)
  USE control_flags, ONLY: rmm_with_davidson
  IMPLICIT NONE
  INTEGER,INTENT(IN) :: iter_ 
  LOGICAL :: res 
  res = (rmm_with_davidson) .AND. ( iter_ < 3 .OR. MOD(iter_,5) == 0) 
END FUNCTION rmm_use_davidson_gipaw

FUNCTION rmm_use_paro_gipaw(iter_) RESULT (res)
  USE control_flags, ONLY: rmm_with_davidson
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: iter_ 
  LOGICAL  :: res 
  res = (.NOT. rmm_with_davidson) .AND.  (MOD(iter_,5) == 1) 
END FUNCTION rmm_use_paro_gipaw
